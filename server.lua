...
-- Override default player slots to 12 (2 rows × 6 columns) for bag system
shared.playerslots = 12
print(string.format('^2[Bag System]^0 Default player slots set to: %d', shared.playerslots))
...
function server.setPlayerInventory(player, data)
	...
	if inv then
		print(string.format('^2[Bag System]^0 Player %s inventory created with %d slots', player.name, inv.slots))
        ...
        -- Update slots if they have bags
        SetTimeout(100, function()
            if exports.ox_inventory then
                exports.ox_inventory:UpdatePlayerBagSlots(player.source)
            end
        end)
	end
end
