...
export const refreshSlotsReducer: CaseReducer<State, PayloadAction<Payload>> = (state, action) => {
  ...
  if (action.payload.slotsData) {
    ...
    // Compare as strings to handle number/string type mismatch from Lua
    const inv =
      String(inventoryId) === String(state.leftInventory.id)
        ? 'leftInventory'
        : String(inventoryId) === String(state.rightInventory.id)
        ? 'rightInventory'
        : null;
