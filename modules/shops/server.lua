...
local function canAccessAce(source, ace)
	return not ace or IsPlayerAceAllowed(source, ace)
end
...
local function registerAdminShop()
	local inventory = {}
	local seen = {}
	...
	for _, item in pairs(Items()) do
		...
		if name and not seen[name] and name ~= 'cash' then
			seen[name] = true
	...
	registerShopType('adminshop', {
		name = 'Admin Shop',
		inventory = inventory,
		ace = 'command.admin-shop',
	})
end
...
lib.callback.register('ox_inventory:openShop', function(source, data)
	...
	if data then
		...
		if not canAccessAce(source, shop.ace) then return end
...
RegisterCommand('admin-shop', function(source)
	if source <= 0 then return end
	TriggerClientEvent('ox_inventory:openAdminShop', source)
end, true)
