...
-- Get the highest bag level in player's inventory
local function GetPlayerBagLevel(inventory)
    if not inventory or not inventory.items then
        return 12 -- Default slots
    end
    
    local highestSlots = 12
    
    for slot, item in pairs(inventory.items) do
        if item and item.name and bags[item.name] then
            local bagSlots = bags[item.name]
            if bagSlots > highestSlots then
                highestSlots = bagSlots
            end
        end
    end
...
-- Check and update player slots based on bags in inventory
local function UpdatePlayerSlots(playerId)
    local inventory = Inventory(playerId)
    
    if not inventory then return end
    
    local requiredSlots = GetPlayerBagLevel(inventory)
    local currentSlots = inventory.slots
    
    if requiredSlots ~= currentSlots then
        -- Check if we're reducing slots and if there are items in slots that will be locked
        if requiredSlots < currentSlots then
            -- Check for items in slots that will be locked
            local hasItemsInLockedSlots = false
            
            for slot, item in pairs(inventory.items) do
                local slotNum = tonumber(slot)
                if slotNum and slotNum > requiredSlots then
                    hasItemsInLockedSlots = true
                    break
                end
            end
            
            if hasItemsInLockedSlots then
                TriggerClientEvent('ox_lib:notify', playerId, {
                    type = 'error',
                    description = 'Cannot remove bag! Items are in locked slots. Move them first.'
                })
                return false
            end
        end
        ...
        Inventory.SetSlotCount(inventory, requiredSlots)
        
        if requiredSlots > currentSlots then
            TriggerClientEvent('ox_lib:notify', playerId, {
                type = 'success',
                description = string.format('Inventory expanded to %d slots (%d rows)!', requiredSlots, math.floor(requiredSlots / 6))
            })
        elseif requiredSlots < currentSlots then
            TriggerClientEvent('ox_lib:notify', playerId, {
                type = 'warning',
                description = string.format('Inventory reduced to %d slots (%d rows)!', requiredSlots, math.floor(requiredSlots / 6))
            })
        end
...
for bagName, slots in pairs(bags) do
    exports(bagName, function(event, item, inventory, slot, data)
        if event == 'usingItem' then
            print(string.format('^3[Bag System Debug]^0 Using %s - Event: %s, Current Slots: %d, Target Slots: %d', bagName, event, inventory.slots, slots))
            
            local currentSlots = inventory.slots
            if currentSlots >= slots then
                print(string.format('^1[Bag System]^0 Player already has %d slots (trying to use %s with %d slots)', currentSlots, bagName, slots))
                TriggerClientEvent('ox_lib:notify', inventory.id, { type = 'error', description = 'You already have a backpack of this level or higher!' })
                return false
            end
            
            -- Upgrade slots
            print(string.format('^2[Bag System]^0 Expanding inventory from %d to %d slots', currentSlots, slots))
            Inventory.SetSlotCount(inventory, slots)
            
            -- Verify the change
            local newSlots = inventory.slots
            print(string.format('^2[Bag System]^0 After SetSlotCount - Inventory now has %d slots', newSlots))
            
            TriggerClientEvent('ox_lib:notify', inventory.id, { 
                type = 'success', 
                description = string.format('Backpack equipped! Inventory expanded to %d slots (%d rows). Keep it in your inventory!', slots, math.floor(slots / 6))
            })
            
            -- DO NOT remove the item - player needs to keep it in inventory
            -- The bag must remain in inventory for the slots to be unlocked
            return false
        end
    end)
end

-- When player closes inventory, verify their slots match their bags to prevent exploits
...
-- Export function to manually update player slots
exports('UpdatePlayerBagSlots', UpdatePlayerSlots)

print('^2[Bag System]^0 Loaded in modules/items/server.lua - Default slots: 12')
