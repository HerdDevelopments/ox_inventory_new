...
local function startInventoryPreview()
	stopInventoryPreview()

	if cache.vehicle or not DoesEntityExist(playerPed) then return end

	local clone = ClonePed(playerPed, GetEntityHeading(playerPed), false, false)
	...
	local cloneCoords = GetOffsetFromEntityInWorldCoords(playerPed, 0.85, 0.85, -1.0)
	SetEntityCoordsNoOffset(previewPed, cloneCoords.x, cloneCoords.y, cloneCoords.z, false, false, false)
	SetEntityHeading(previewPed, GetEntityHeading(playerPed) + 180.0)

	local idleDict = 'amb@world_human_hang_out_street@male_c@idle_a'
	local idleClip = 'idle_a'
	lib.requestAnimDict(idleDict)
	TaskPlayAnim(previewPed, idleDict, idleClip, 8.0, -8.0, -1, 1, 0.0, false, false, false)

	local camCoords = GetOffsetFromEntityInWorldCoords(previewPed, 0.95, 1.65, 0.60)
	previewCam = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', camCoords.x, camCoords.y, camCoords.z, 0.0, 0.0, 0.0, 40.0, true, 2)
	...
	PointCamAtEntity(previewCam, previewPed, 0.0, 0.0, 0.55, true)
...
RegisterNetEvent('ox_inventory:openAdminShop', function()
	if source == '' then return end
	client.openInventory('shop', { type = 'adminshop' })
end)
